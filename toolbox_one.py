...
def count_line_file(type_file='*.txt', *directories):
    """Count line file.

        Find in a list of directories and listed
        the number of files per directory and
        listed the number of lines per file.
        
        TODO: improve the presentation of code
               by prompt.
               
        Use:
        count_line_file('*.txt','dir1/','dir2/')
    ...
    for directory in directories:
        list_files = glob.glob(directory+type_file)
        current_file = 0

        while current_file < len(list_files):
            file = open(list_files[current_file]).readlines()
            
            for name in list_files:
                name = name.replace(type_file,'')
            for name in list_files:
                print(name)
                print(len(file))
            current_file += 1
